#!/usr/bin/env python3
"""Shared hardened HTTP transport for trusted AI review integrations."""
...
def _validate_limits(timeout: int, max_attempts: int, max_response_bytes: int) -> None:
    ...
    if isinstance(max_response_bytes, bool) or not isinstance(max_response_bytes, int) or not 1 <= max_response_bytes <= MAX_RESPONSE_BYTES_LIMIT:
        raise ValueError(f"max_response_bytes must be an integer in 1..{MAX_RESPONSE_BYTES_LIMIT}")
...
def post_json(
    ...
    *,
    ...
    """POST bounded JSON; reject redirects and never log provider response bodies."""
    ...
    for attempt in range(max_attempts):
        request = Request(safe_endpoint, data=encoded, headers={"Content-Type": "application/json"}, method="POST")
        ...
        try:
            with opener(request, timeout=timeout) as response:
                ...
                if len(raw) > max_response_bytes:
                    raise ProviderResponseError(f"{provider_name} response exceeded {max_response_bytes} bytes")
        ...
        print(f"{provider_name} transient failure; retrying attempt {attempt + 2}/{max_attempts} after {delay:.1f}s", file=sys.stderr)
        sleeper(delay)
