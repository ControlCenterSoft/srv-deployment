...
build_runtime() {
  ...
  (cd "$ROOT" && CGO_ENABLED=0 GOOS=linux GOARCH="$goarch" go build -trimpath -buildvcs=false \
...
# Signed downgrade rejection below stable 1.0.0.
...
GO
CGO_ENABLED=0 GOOS=linux GOARCH="$goarch" go build -trimpath -buildvcs=false -o "$work/broken" "$work/broken.go"
...
# Known-good must restart immediately after rollback without manual reset-failed.
