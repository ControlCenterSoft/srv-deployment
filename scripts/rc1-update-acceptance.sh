...
fail() { printf 'RC1 UPDATE ACCEPTANCE FAILED: %s\n' "$*" >&2; exit 1; }
...
p=sys.argv[1]
d=json.load(open(p))
d["channel"]="stable" if d.get("channel") != "stable" else "rc"
...
printf 'RC1 update acceptance passed: signed forward update, fail-closed negative verification, automatic rollback, immediate post-rollback restart.\n'
