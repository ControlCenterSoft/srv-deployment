...
func TestNetworkAddressPreviewRequiresAdminAndCSRF(t *testing.T) {
	app := newTestApp(t)
	payload := map[string]any{"interface": "definitely-missing", "cidr": "192.0.2.10/24"}

	anonymous := requestJSON(t, app.handler, http.MethodPost, "/api/v1/network/address-change/preview", payload, nil, "")
	...
	missingCSRF := requestJSON(t, app.handler, http.MethodPost, "/api/v1/network/address-change/preview", payload, adminCookie, "")
	if missingCSRF.Code != http.StatusForbidden {
		t.Fatalf("missing csrf status=%d body=%s", missingCSRF.Code, missingCSRF.Body.String())
	}

	invalid := requestJSON(t, app.handler, http.MethodPost, "/api/v1/network/address-change/preview", payload, adminCookie, csrf)
