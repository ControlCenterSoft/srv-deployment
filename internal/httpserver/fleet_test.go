...
func TestFleetInventoryCreateListAndViewerRBAC(t *testing.T) {
	...
	if rr.Code != http.StatusCreated || !strings.Contains(rr.Body.String(), `"status":"pending_enrollment"`) {
	...
	if rr.Code != http.StatusOK {
		t.Fatalf("viewer fleet read=%d %s", rr.Code, rr.Body.String())
	}
...
func TestFleetMutationRequiresCSRF(t *testing.T) {
	...
	if rr.Code != http.StatusForbidden {
		t.Fatalf("missing enrollment csrf=%d", rr.Code)
	}
}
...
func TestFleetSecureEnrollmentHeartbeatAndHealth(t *testing.T) {
	...
	if enrolled.AgentCredential == "" {
		t.Fatal("missing one-time agent credential")
	}
	if strings.Contains(rr.Body.String(), "agent_credential_hash") {
		t.Fatal("agent hash leaked")
	}

	heartbeat := `{"node_id":"srv-01","agent_version":"1.1.9","hostname":"srv-01.example","os_name":"Ubuntu","os_version":"26.04","architecture":"amd64"}`
	rr = requestJSON(t, app.handler, http.MethodPost, "/api/v1/fleet/heartbeat", heartbeat, nil, "")
	if rr.Code != http.StatusUnauthorized {
		t.Fatalf("heartbeat without bearer=%d %s", rr.Code, rr.Body.String())
	}

	req := httptest.NewRequest(http.MethodPost, "/api/v1/fleet/heartbeat", strings.NewReader(heartbeat))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+enrolled.AgentCredential)
	rr = httptest.NewRecorder()
	app.handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK || !strings.Contains(rr.Body.String(), `"health":"healthy"`) {
	...
	if strings.Contains(rr.Body.String(), "agent_credential_hash") || strings.Contains(rr.Body.String(), enrolled.AgentCredential) {
		t.Fatal("credential leaked in fleet list")
	}
}
