(() => {
    ...
    function renderInterfaces(interfaces) {
        ...
        for (const item of interfaces || []) {
            ...
            if (item.ipv4 && item.ipv4.length) lines.push(`IPv4: ${item.ipv4.join(", ")}`);
    ...
    function initializePlanner(data) {
        ...
        const lan = (data.lan_candidates || [])[0]
        ...
        if (ipv4 && ipv4.address && ipv4.prefix) {
    ...
    async function refreshOverview() {
        ...
        if (!response.ok || payload.ok !== true) throw new Error("network overview request failed");
    ...
    async function refreshDiagnostics() {
        ...
        const payload = await response.json();
        if (!response.ok || payload.ok !== true) throw new Error("network diagnostics request failed");
        renderDiagnostics(payload.data || {});
    }

    async function refreshAll() {
        try {
            await Promise.all([refreshOverview(), refreshDiagnostics()]);
            updateLive(true);
        } catch (_) {
            updateLive(false);
        }
    ...
    async function submitPlan(event) {
        ...
        try {
            const response = await fetch("/api/v1/network/plan", {
                method: "POST",
                cache: "no-store",
            ...
            const payload = await response.json();
            if (!response.ok) throw new Error(payload.error || "network validation request failed");
            renderPlan(payload);
        } catch (error) {
            const panel = byId("networkPlanResultPanel");
            const status = byId("networkPlanStatus");
