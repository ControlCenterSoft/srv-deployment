...
if current < (1, 0, 0) or current > (1, 1, 0):
    raise SystemExit(
        f"release 1.1.0 supports upgrade from 1.0.0 and revalidation of 1.1.0; installed={version}"
    )
PY
...
grep -Fq 'Права пользователей' "$PAYLOAD/templates/shell.html" || fail "user-rights menu entry missing"
grep -Fq 'Торренты' "$PAYLOAD/templates/shell.html" || fail "Torrents menu entry missing"
grep -Fq 'AdGuard VPN' "$PAYLOAD/templates/shell.html" || fail "AdGuard VPN menu entry missing"
grep -Fq 'Сервисы' "$PAYLOAD/templates/shell.html" || fail "Services menu entry missing"
