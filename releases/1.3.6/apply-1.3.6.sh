...
TMP="$(mktemp "${SOURCE_DIR}/.apply-1.3.6.XXXXXX.sh")"
...
cleanup(){ rm -f -- "$TMP"; }
trap cleanup EXIT
fail(){ printf 'APPLY 1.3.6 FAIL: %s\n' "$*" >&2; exit 1; }
warn(){ printf 'APPLY 1.3.6 WARN: %s\n' "$*" >&2; }
...
if "1.3.3" not in text:
    raise SystemExit("1.3.6 inherited apply identity anchor missing")
...
if old not in text:
    raise SystemExit("1.3.6 legacy timer hard-gate anchor missing")
...
if old not in text:
    raise SystemExit("1.3.6 legacy updater hard-gate anchor missing")
text=text.replace(old,new,1)
...
# Minecraft is a managed optional subsystem. Try to recover its established
# legacy schedule, but never roll back a healthy Control Center deployment just
# because this independent game-server updater is unhealthy.
...
data['version']='1.3.6'
data['release_id']='1.3.6'
if sys.argv[2] and sys.argv[2] != 'unknown':
    data['git_sha']=sys.argv[2]
fd,tmp=tempfile.mkstemp(prefix='.release-1.3.6.',dir=str(path.parent))
try:
...
printf 'APPLY 1.3.6 PASS: core deployment completed; GitHub schedule restored; Minecraft health isolated\n'
