(() => {
    ...
    async function promptAction(text, options = {}) {
        if (window.ControlCenterUI?.prompt) return window.ControlCenterUI.prompt(text, options);
    ...
    async function serviceAction(action) {
        if (action !== "start") {
            const approved = await confirmAction(
                `${action === "stop" ? "Остановить" : "Перезапустить"} Minecraft сервер?`,
                {title: action === "stop" ? "Остановка Minecraft" : "Перезапуск Minecraft", confirmLabel: action === "stop" ? "Остановить" : "Перезапустить", danger: action === "stop"}
            );
            if (!approved) return;
        }
        setBusy(true); message("Выполняется команда управления сервером…");
        try {
            const d = await write(`${BASE}/service/${action}`);
            message(d.message || "Команда выполнена.");
            toast(d.message || "Команда Minecraft выполнена.", "success");
            await loadOverview();
        }
        catch (e) { message(`Ошибка управления: ${e.message}`, true); toast(e.message, "error"); setBusy(false); }
    }

    async function updateNow() {
        const approved = await confirmAction(
            "Проверить актуальную версию Minecraft Bedrock и при необходимости обновить сервер сейчас? Перед обновлением будет выполнена штатная процедура защиты данных.",
            {title: "Обновление Minecraft Bedrock", confirmLabel: "Проверить и обновить"}
        );
        if (!approved) return;
        setBusy(true); message("Проверка и обновление Minecraft выполняются. Это может занять несколько минут…");
        try {
            const d = await write(`${BASE}/update`);
            message(d.message || "Проверка обновления завершена.");
            toast(d.message || "Проверка обновления завершена.", "success");
            await Promise.all([loadOverview(), loadBackups(), loadLogs()]);
        }
        catch (e) { message(`Ошибка обновления: ${e.message}`, true); toast(e.message, "error"); setBusy(false); }
    }

    async function backupNow() {
        setBusy(true); message("Создаётся резервная копия Minecraft…");
        try {
            const d = await write(`${BASE}/backup`);
            message(d.message || "Резервная копия создана.");
            toast(d.message || "Резервная копия Minecraft создана.", "success");
            await loadBackups(); await loadOverview();
        }
        catch (e) { message(`Ошибка backup: ${e.message}`, true); toast(e.message, "error"); setBusy(false); }
    }

    async function broadcast() {
        const value = $("broadcastMessage").value.trim(); if (!value) return message("Введите сообщение для игроков.", true);
        setBusy(true);
        try { const d = await write(`${BASE}/live/say`, "POST", {message:value}); $("broadcastMessage").value=""; message(d.message || "Сообщение отправлено."); toast("Сообщение отправлено игрокам.", "success"); }
        catch(e){ message(`Ошибка live-консоли: ${e.message}`, true); toast(e.message, "error"); }
    ...
    async function saveSettings(event) {
        ...
        try { const d=await write(`${BASE}/properties`, "PUT", {changes}); message(d.message || "Параметры сохранены."); toast("Параметры Minecraft сохранены.", "success"); await Promise.all([loadSettings(), loadOverview()]); }
        catch(e){ message(`Ошибка сохранения: ${e.message}`, true); toast(e.message, "error"); setBusy(false); }
    }
    ...
    async function loadPlayers() {
        ...
        if (!players.length) { body.innerHTML='<tr><td colspan="5" class="mc132-dim">Игроки ещё не обнаружены.</td></tr>'; enhanceTables(); return; }
    ...
    async function addAllow() {
        ...
        try { await write(`${BASE}/players/allowlist`,"POST",{name,xuid,ignores_player_limit:false}); if(xuid&&permission) await write(`${BASE}/players/${encodeURIComponent(xuid)}/permission`,"PUT",{permission}); $("newPlayerName").value=""; $("newPlayerXuid").value=""; message("Игрок добавлен."); toast("Игрок добавлен в Allow-list.", "success"); await loadPlayers(); }
        catch(e){message(`Ошибка игрока: ${e.message}`,true);toast(e.message,"error");} finally{setBusy(false);}
    }
    async function quickAllow(player){setBusy(true);try{await write(`${BASE}/players/allowlist`,"POST",{name:player.name,xuid:player.xuid||"",ignores_player_limit:false});message("Игрок добавлен в Allow-list.");toast("Игрок добавлен в Allow-list.","success");await loadPlayers();}catch(e){message(e.message,true);toast(e.message,"error");}finally{setBusy(false);}}
    async function removeAllow(key){const approved=await confirmAction(`Убрать ${key} из Allow-list?`,{title:"Изменение Allow-list",confirmLabel:"Убрать",danger:true});if(!approved)return;setBusy(true);try{await write(`${BASE}/players/allowlist/${encodeURIComponent(key)}`,"DELETE");message("Игрок удалён из Allow-list.");toast("Игрок удалён из Allow-list.","success");await loadPlayers();}catch(e){message(e.message,true);toast(e.message,"error");}finally{setBusy(false);}}
    async function savePermission(xuid,permission){setBusy(true);try{if(permission)await write(`${BASE}/players/${encodeURIComponent(xuid)}/permission`,"PUT",{permission});else await write(`${BASE}/players/${encodeURIComponent(xuid)}/permission`,"DELETE");message("Права игрока обновлены.");toast("Права игрока обновлены.","success");await loadPlayers();}catch(e){message(e.message,true);toast(e.message,"error");}finally{setBusy(false);}}
    async function livePlayer(action,username){if(action==="kick"){const approved=await confirmAction(`Отключить игрока ${username}?`,{title:"Отключение игрока",confirmLabel:"Отключить",danger:true});if(!approved)return;}setBusy(true);try{const body={username};if(action==="kick")body.reason="Отключён администратором";await write(`${BASE}/live/${action}`,"POST",body);message(`${action.toUpperCase()} выполнен для ${username}.`);toast(`${action.toUpperCase()} выполнен для ${username}.`,"success");setTimeout(()=>loadPlayers().catch(()=>{}),1000);}catch(e){message(e.message,true);toast(e.message,"error");}finally{setBusy(false);}}

    async function loadWorlds(){const d=await api(`${BASE}/worlds`);state.canWrite=Boolean(d.can_write??state.canWrite);const body=$("worldsBody");body.replaceChildren();const worlds=firstArray(d,"worlds");if(!worlds.length){body.innerHTML='<tr><td colspan="5" class="mc132-dim">Миры не найдены.</td></tr>';enhanceTables();return;}for(const world of worlds){const tr=document.createElement("tr");tr.innerHTML=`<td><strong>${esc(world.name)}</strong></td><td>${esc(world.size_human||humanBytes(world.size))}</td><td>${esc(world.modified||"—")}</td><td>${world.active?'<span class="mc-badge ok">Активный</span>':'<span class="mc-badge">—</span>'}</td><td></td>`;const a=tr.lastElementChild;a.className="mc132-row-actions";a.append(rowButton("Активировать","secondary-button",()=>worldAction("activate",world.name),world.active),rowButton("Клонировать","secondary-button",()=>worldAction("clone",world.name)),rowButton("Переименовать","secondary-button",()=>worldAction("rename",world.name,world.active)),rowButton("Удалить","danger-button",()=>worldAction("delete",world.name),world.active));body.appendChild(tr);}enhanceTables();}
    async function worldAction(action,name,active=false){let body,method="POST",url=`${BASE}/worlds/${encodeURIComponent(name)}/${action}`;if(action==="activate"){const approved=await confirmAction(`Активировать мир «${name}»? Minecraft будет перезапущен.`,{title:"Активация мира",confirmLabel:"Активировать"});if(!approved)return;}if(action==="clone"||action==="rename"){if(action==="rename"&&active){const approved=await confirmAction("Это активный мир. Minecraft может потребовать перезапуск после переименования. Продолжить?",{title:"Переименование активного мира",confirmLabel:"Продолжить",danger:true});if(!approved)return;}const nn=await promptAction(`${action==="clone"?"Укажите имя копии":"Укажите новое имя"} мира «${name}».`,{title:action==="clone"?"Клонирование мира":"Переименование мира",defaultValue:action==="clone"?`${name} - копия`:name,inputLabel:"Имя мира"});if(nn===null||!nn.trim())return;body={new_name:nn.trim()};}if(action==="delete"){const approved=await confirmAction(`Удалить мир «${name}»? Это необратимая операция.`,{title:"Удаление мира",confirmLabel:"Удалить мир",danger:true,requireText:name});if(!approved)return;method="DELETE";url=`${BASE}/worlds/${encodeURIComponent(name)}`;body={confirm:name};}setBusy(true);try{const d=await write(url,method,body);message(d.message||"Операция с миром выполнена.");toast(d.message||"Операция с миром выполнена.","success");await Promise.all([loadWorlds(),loadSettings(),loadOverview()]);}catch(e){message(`Ошибка мира: ${e.message}`,true);toast(e.message,"error");}finally{setBusy(false);}}

    async function loadBackups(){const d=await api(`${BASE}/backups`);state.canWrite=Boolean(d.can_write??state.canWrite);const body=$("backupsBody");body.replaceChildren();const backups=firstArray(d,"backups");if(!backups.length){body.innerHTML='<tr><td colspan="6" class="mc132-dim">Резервные копии не найдены.</td></tr>';enhanceTables();return;}for(const b of backups){const tr=document.createElement("tr");const valid=b.valid!==false;tr.innerHTML=`<td><strong>${esc(b.name||b.id||"—")}</strong></td><td>${esc(b.size_human||humanBytes(b.size))}</td><td>${esc(b.mtime||b.created_at||"—")}</td><td>${esc((b.contents||[]).join(", "))}</td><td>${valid?'<span class="mc-badge ok">OK</span>':'<span class="mc-badge error">Ошибка</span>'}</td><td></td>`;const a=tr.lastElementChild;a.className="mc132-row-actions";const name=b.name||b.id;if(valid)a.appendChild(rowButton("Восстановить","secondary-button",()=>restoreBackup(name)));a.appendChild(rowButton("Удалить","danger-button",()=>deleteBackup(name)));body.appendChild(tr);}enhanceTables();}
    async function restoreBackup(name){const approved=await confirmAction(`Восстановить Minecraft из backup «${name}»? Текущий сервер будет остановлен на время восстановления.`,{title:"Восстановление Minecraft",confirmLabel:"Восстановить",danger:true});if(!approved)return;setBusy(true);message("Восстановление Minecraft…");try{const d=await write(`${BASE}/backups/${encodeURIComponent(name)}/restore`);message(d.message||"Восстановление завершено.");toast(d.message||"Восстановление Minecraft завершено.","success");await Promise.all([loadOverview(),loadBackups(),loadWorlds(),loadSettings()]);}catch(e){message(`Ошибка восстановления: ${e.message}`,true);toast(e.message,"error");}finally{setBusy(false);}}
    async function deleteBackup(name){const approved=await confirmAction(`Удалить backup «${name}»?`,{title:"Удаление backup Minecraft",confirmLabel:"Удалить",danger:true,requireText:name});if(!approved)return;setBusy(true);try{const d=await write(`${BASE}/backups/${encodeURIComponent(name)}`,"DELETE",{confirm:name});message(d.message||"Backup удалён.");toast(d.message||"Backup удалён.","success");await loadBackups();}catch(e){message(e.message,true);toast(e.message,"error");}finally{setBusy(false);}}
    ...
    async function loadAll(){message("Обновление данных…");try{await loadOverview();const active=document.querySelector(".mc132-tab.active")?.dataset.tab||"overview";if(active==="settings")await loadSettings();if(active==="players")await loadPlayers();if(active==="worlds")await loadWorlds();if(active==="backups")await loadBackups();if(active==="logs")await loadLogs();message("");}catch(e){message(`Ошибка загрузки Minecraft: ${e.message}`,true);toast(e.message,"error");setBusy(false);}}
    async function openTab(name){document.querySelectorAll(".mc132-tab").forEach((b)=>{const active=b.dataset.tab===name;b.classList.toggle("active",active);b.setAttribute("aria-selected",active?"true":"false");b.tabIndex=active?0:-1;});document.querySelectorAll(".mc132-tab-panel").forEach((p)=>p.classList.toggle("active",p.dataset.panel===name));try{if(name==="settings")await loadSettings();else if(name==="players")await loadPlayers();else if(name==="worlds")await loadWorlds();else if(name==="backups")await loadBackups();else if(name==="logs")await loadLogs();else await loadOverview();}catch(e){message(`Ошибка раздела: ${e.message}`,true);toast(e.message,"error");}}

    const tabs=Array.from(document.querySelectorAll(".mc132-tab"));
    tabs.forEach((b,index)=>{b.setAttribute("role","tab");b.setAttribute("aria-selected",b.classList.contains("active")?"true":"false");b.tabIndex=b.classList.contains("active")?0:-1;b.addEventListener("click",()=>openTab(b.dataset.tab));b.addEventListener("keydown",(event)=>{if(!["ArrowLeft","ArrowRight","Home","End"].includes(event.key))return;event.preventDefault();let next=index;if(event.key==="ArrowRight")next=(index+1)%tabs.length;else if(event.key==="ArrowLeft")next=(index-1+tabs.length)%tabs.length;else if(event.key==="Home")next=0;else if(event.key==="End")next=tabs.length-1;tabs[next].focus();openTab(tabs[next].dataset.tab);});});
    document.querySelector(".mc132-tabs")?.setAttribute("role","tablist");
    ...
    enhanceTables();
    (async()=>{try{await loadAuth();await loadOverview();message("");window.setInterval(()=>{if(!state.busy&&document.visibilityState==="visible")loadOverview().catch(()=>{});},15000);}catch(e){message(`Minecraft недоступен: ${e.message}`,true);toast(e.message,"error");}})();
})();
