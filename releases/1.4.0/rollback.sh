...
for p in /usr/local/libexec/srv-control-backup /usr/local/libexec/srv-control-release14-agent /usr/local/libexec/srv-control-pxe-probe /etc/systemd/system/srv-control-release14-agent.service /etc/systemd/system/srv-control-release14-agent.path /etc/systemd/system/srv-control-backup-retention.service /etc/systemd/system/srv-control-backup-retention.path; do restore_abs "$p"; done
...
log "ROLLBACK PASS: restored pre-1.4.0 application, backup worker and schema"
