...
if [[ -f "$RELEASE_META" ]]; then
    cp -a "$RELEASE_META" "$BACKUP_DIR/state/release.json"
else
    : > "$BACKUP_DIR/state/release.json.absent"
fi
...
PY
then
    systemctl enable --now srv-control-backup.timer
else
...
PY
then
    systemctl enable --now srv-control-os-auto-update.timer
else
