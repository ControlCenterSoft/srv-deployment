...
[[ "$ORIGINAL_SOURCE" == "1.3.8" || "$ORIGINAL_SOURCE" == "2.0.0" || "$ORIGINAL_SOURCE" == "2.1.0" ]] \
    || fail "rollback source version is invalid: $ORIGINAL_SOURCE"
...
# Stop 2.1.1-owned execution paths before restoring service identity and files.
...
for raw in meta.read_text(encoding='utf-8').splitlines():
    ...
    row=json.loads(raw); rel=row['path']
...
if [[ "$ORIGINAL_SOURCE" == "2.1.0" ]]; then
    ...
    # Prove the restored frozen 2.1.0 control path is still healthy without
    # changing the world or performing another package update.
    /usr/local/sbin/srv-control-minecraft status > /tmp/srvcc-2.1.1-rollback-status.json \
        || { cat /tmp/srvcc-2.1.1-rollback-status.json >&2 || true; fail "restored 2.1.0 Minecraft status failed"; }
...
PY
    ...
    # Unwind the exact frozen 2.1.0 transaction that this patch chained first.
    bash "$BASE_RELEASE/rollback-2.1.0.sh" "$PROJECT" "$REMOTE_SHA" \
        || fail "frozen 2.1.0 rollback failed while returning to $ORIGINAL_SOURCE"
fi

# Remove an account created solely by 2.1.1 only after all services and metadata
# have been restored. Failure to remove it is non-fatal; availability wins.
if [[ "$(cat "$BACKUP_DIR/state/target-user-existed" 2>/dev/null || echo yes)" == "no" ]]; then
    if command -v userdel >/dev/null 2>&1; then userdel "$TARGET_USER" >/dev/null 2>&1 || warn "temporary minecraft account could not be removed"; fi
fi
if [[ "$(cat "$BACKUP_DIR/state/target-group-existed" 2>/dev/null || echo yes)" == "no" ]]; then
    if command -v groupdel >/dev/null 2>&1; then groupdel "$TARGET_GROUP" >/dev/null 2>&1 || warn "temporary minecraft group could not be removed"; fi
fi

printf 'ROLLBACK 2.1.1 PASS: restored source=%s; runtime metadata and scheduler state replayed\n' "$ORIGINAL_SOURCE"
