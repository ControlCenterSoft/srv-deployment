...
PREVIOUS_ROLLBACK="${RELEASE_DIR}/../1.3.0/rollback.sh"
BACKUP_ROOT="/var/lib/srv-deployment/backups"
CURRENT_BACKUP="${BACKUP_ROOT}/${REMOTE_SHA}-${RELEASE_ID}"
COMPAT_BACKUP="${BACKUP_ROOT}/${REMOTE_SHA}-1.3.0"

fail() { printf 'ROLLBACK FAIL: %s\n' "$*" >&2; exit 1; }
[[ -s "$PREVIOUS_ROLLBACK" ]] || fail "validated 1.3.0 rollback implementation missing"
