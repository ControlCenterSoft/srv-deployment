...
AGENT_LOG="/var/log/srvcc-agent.log"
...
[[ -d "$PAYLOAD/app" ]] || fail "installer payload is missing"
[[ -s "$PAYLOAD/requirements.lock" ]] || fail "requirements.lock is missing"
command -v apt-get >/dev/null 2>&1 \
    || fail "Debian/Ubuntu with apt-get is required"
command -v systemctl >/dev/null 2>&1 \
    || fail "systemd is required"

if [[ -e "$PROJECT" ]] && find "$PROJECT" -mindepth 1 -print -quit 2>/dev/null | grep -q .; then
    fail "$PROJECT already contains files; this clean-install installer will not overwrite an existing Control Center"
fi
...
log "Installing SRV Control Center application snapshot"
...
if ! runuser -u postgres -- psql -tAc \
    "SELECT 1 FROM pg_roles WHERE rolname='srv-control'" \
    | grep -q 1
then
...
if ! runuser -u postgres -- psql -tAc \
    "SELECT 1 FROM pg_database WHERE datname='srv_control'" \
    | grep -q 1
then
...
log "Configuring local reverse proxy"
web_port=80
...
ln -sfn \
    ...
    /etc/nginx/sites-enabled/srv-control
...
log "Installing automatic GitHub updater"
rm -rf "$DEPLOY_REPO"
git clone \
    --no-tags \
    --single-branch \
...
[[ -x "$DEPLOY_REPO/deploy/deploy.sh" ]] \
    || { log "deploy/deploy.sh is missing."; exit 1; }
...
cat > /etc/systemd/system/srvcc-github-agent.service <<EOF
[Unit]
...
cat > /etc/systemd/system/srvcc-github-agent.timer <<'EOF'
[Unit]
Description=Check GitHub for SRV Control Center updates
...
Persistent=true
Unit=srvcc-github-agent.service
...
touch "$AGENT_LOG"
chmod 0640 "$AGENT_LOG"

systemctl daemon-reload
systemctl enable srv-control.service
systemctl enable srvcc-github-agent.timer
systemctl restart srv-control.service
systemctl restart nginx.service
systemctl start srvcc-github-agent.timer
...
systemctl is-active --quiet srvcc-github-agent.timer \
    || fail "GitHub update timer is not active"
