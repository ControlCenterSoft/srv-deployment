...
def _validate_dhcp(body):
    ...
    if start in (net.network_address,net.broadcast_address) or end in (net.network_address,net.broadcast_address):raise ValueError('Диапазон не может включать адрес сети или broadcast')
    ...
    try:lease=int(body.get('lease_minutes',720))
    except Exception:raise ValueError('Срок аренды должен быть числом минут')
...
def _remote_release():
    try:
        req=urllib.request.Request(DEPLOYMENT_URL,headers={'User-Agent':'Control-Center/1.0.4'}); data=json.loads(urllib.request.urlopen(req,timeout=4).read().decode()); return {'available':True,'release':data.get('release'),'channel':data.get('channel','production')}
