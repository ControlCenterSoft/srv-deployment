...
def _normalized_update_settings():
    ...
    if 'interval_minutes' not in raw:
        ...
        raw['interval_minutes']=legacy.get(raw.get('frequency'),60)
    try: interval=int(raw.get('interval_minutes',60))
...
def _top_processes():
    try:
        ...
        for line in out.splitlines()[:3]:
            ...
            if len(parts)==4:
                result.append({'pid':int(parts[0]),'name':parts[1],'cpu_percent':float(parts[2]),'memory_percent':float(parts[3])})
        return result
...
def _storage():
    try:
        ...
        for line in out.splitlines()[1:]:
            ...
            if len(p)<6: continue
            try:
                total=int(p[1]); used=int(p[2]); available=int(p[3]); percent=float(p[4].rstrip('%'))
            except Exception: continue
            rows.append({'device':p[0],'mount':p[5],'total':total,'used':used,'available':available,'percent':percent})
...
def _wan_telemetry():
    cfg=_read_json(NETWORK_FILE,{})
    name=(cfg.get('wan') or {}).get('interface')
    interfaces={x['name']:x for x in _interfaces()}
    row=interfaces.get(name)
    if row:return {'interface':name,'state':row['state'],'rx_bytes':row['rx_bytes'],'tx_bytes':row['tx_bytes'],'ipv4':row['ipv4']}
    return {'interface':name or '', 'state':'unassigned','rx_bytes':0,'tx_bytes':0,'ipv4':[]}
...
def _validate_network(body):
    ...
    known={x['name'] for x in _interfaces()}; result={'wan':{},'lan':{}}; used=set(); static_nets=[]
    for role in ('wan','lan'):
        ...
        if method=='static':
            ...
            static_nets.append((role,addr.network)); dst.update({'ip':str(addr.ip),'mask':prefix,'gateway':gateway,'dns':clean_dns})
        result[role]=dst
...
def _remote_release():
    try:
        req=urllib.request.Request(DEPLOYMENT_URL,headers={'User-Agent':'Control-Center/1.0.3'})
...
def update_settings():
    ...
    if request.method=='POST':
        body=request.get_json(silent=True) or {}; automatic=body.get('automatic_updates'); channel=body.get('channel','production')
